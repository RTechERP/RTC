...
namespace BMS
{
    public partial class frmJobRequirementDetails : _Forms
    {
        ...
        public frmJobRequirementDetails()
        {
            InitializeComponent();
        }
        ...
        void SendMail(string fullName, JobRequirementModel jobRequirement)
        {
            if (jobRequirement.ID <= 0) return;
            EmployeeSendEmailModel sendEmail = new EmployeeSendEmailModel();

            EmployeeModel employee = SQLHelper<EmployeeModel>.FindByID(TextUtils.ToInt(jobRequirement.ApprovedTBPID));
            ...
            sendEmail.EmployeeID = TextUtils.ToInt(jobRequirement.EmployeeID); //Người gửi
            sendEmail.Receiver = TextUtils.ToInt(jobRequirement.ApprovedTBPID); //Người nhận
