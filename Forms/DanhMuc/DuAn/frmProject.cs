...
using Forms.DanhMuc.DuAn.Project;
using Forms.DanhMuc.DuAn.ProjectItem;
using System;
...
namespace BMS
{
    public partial class frmProject : _Forms
    {
        ...
        private void frmProject_Load(object sender, EventArgs e)
        {
            ...
            dtpFromDate.Value = new DateTime(DateTime.Now.Year, 1, 1, 0, 0, 0).AddYears(-1);
            loadFolder();
            ...
            //btnProjectItemNewContent.Visible = Global.IsAdmin;
        ...
        void LoadProjectStatus()
        {
            ...
            cboProjectStatusNew.Properties.DisplayMember = "StatusName";
            cboProjectStatusNew.Properties.ValueMember = "ID";
            cboProjectStatusNew.Properties.DataSource = list;
        }
        ...
        void loadProject()
        {
            using (WaitDialogForm fWait = new WaitDialogForm())
            {
                ...
                string projectStatus = TextUtils.ToString(cboProjectStatusNew.EditValue);
                if (string.IsNullOrWhiteSpace(projectStatus))
                {
                    var listStatus = SQLHelper<ProjectStatusModel>.FindAll().Select(p => p.ID).ToList();
                    projectStatus = string.Join(",", listStatus);
                }


                DataSet oDataSet = TextUtils.LoadDataSetFromSP("spGetProject"
                    , new string[] { "@PageSize", "@PageNumber", "@DateStart", "@DateEnd", "@FilterText", "@CustomerID", "@UserID", "@ListProjectType", "@LeaderID", "@UserIDTech", "@EmployeeIDPM", "@1", "@2", "@3", "@4", "@5", "@6", "@7", "@8", "@9", "@UserIDPriotity", "@BusinessFieldID", "@ProjectStatus" }
                    , new object[] { TextUtils.ToInt(txtPageSize.Text), TextUtils.ToInt(txtPageNumber.Text), dateTimeS, dateTimeE, txtFilterText.Text.Trim()
                                ,TextUtils.ToInt(cbCustomer.EditValue),TextUtils.ToInt(cbUser.EditValue), projectType, TextUtils.ToInt(cboLeader.EditValue), TextUtils.ToInt(cboUserTech.EditValue),employeePm ,typeCheck[0] ,typeCheck[1] ,typeCheck[2] ,typeCheck[3] ,typeCheck[4] ,typeCheck[5] ,typeCheck[6] ,typeCheck[7] ,typeCheck[8],Global.UserID, _bussinessFieldID,projectStatus});
                grdMaster.DataSource = oDataSet.Tables[0];

                if (oDataSet.Tables.Count == 0) return;
                txtTotalPage.Text = TextUtils.ToString(oDataSet.Tables[1].Rows[0]["TotalPage"]);
                lblShowCount.Text = TextUtils.ToString(oDataSet.Tables[2].Rows[0]["TotalEntries"]) + " Entries";

                //var exp = new Expression("ID", "4,5,9,12", "IN");
                //List<ProjectStatusModel> listStatus = SQLHelper<ProjectStatusModel>.FindByExpression(exp);
        ...
        private void CreateDirectoryWithDatatable(DataRow row, string path)
        {
            ...
            else
            {
                ...
                {
                    if (string.IsNullOrEmpty(TextUtils.ToString(parentFolder[0]["Path"])))
                    {
                        CreateDirectoryWithDatatable(parentFolder[0], path);
                    }
                    else
        ...
        private void oPENFILELOCATIONToolStripMenuItem_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                //string pathLocation = @"\\rtctechnologydata.ddns.net\DUAN\Projects\";

                //string path = $@"{Global.PathLocationProject}\{year}\{code}";
                ...
                ////Try get windows credential 
                //try
                //{
                //    NetworkCredential credential = new NetworkCredential();
                //    var a = credential.GetCredential(new Uri(@"\\192.168.1.190"),"basic");
                //    //var a = credential.Password;
                ...
                if (listProjectTypeID.Count > 0)
                {
                    for (int i = 0; i < listProjectTypeID.Count; i++)
                    {
                        ...
                        DataTable dt = TextUtils.LoadDataFromSP("sp_GetProjectTypeTreeFolder", "A",
                                                                new string[] { "@ProjectTypeID" },
                                                                new object[] { listProjectTypeID[i] });
                ...
                try
                {
                    ...
                    {
                        string pathItem = $@"{path}\TaiLieuChung\{item.FolderName}";
                        Directory.CreateDirectory(pathItem);
                        var listSubFolders = list = SQLHelper<ProjectTreeFolderModel>.FindByAttribute("ParentID", item.ID);
                        foreach (var subItem in listSubFolders)
                        {
                            string pathSubItem = $@"{path}\TaiLieuChung\{item.FolderName}\{subItem.FolderName}";
                            Directory.CreateDirectory(pathSubItem);
                        }
                    }
        ...
        private void grvProjectItem_RowStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowStyleEventArgs e)
        {
            if (e.RowHandle >= 0)
            {
                ...
                int totalDayExpridSoon = TextUtils.ToInt(grvProjectItem.GetRowCellValue(e.RowHandle, "TotalDayExpridSoon"));

                DateTime? dateEndActual = TextUtils.ToDate4(grvProjectItem.GetRowCellValue(e.RowHandle, "ActualEndDate"));
                if (itemLate == 1)
                {
                ...
                else if (totalDayExpridSoon <= 3 && !dateEndActual.HasValue)
                {
        ...
        private void btnUpdateLeader_Click(object sender, EventArgs e)
        {
            int rowHandle = grvMaster.FocusedRowHandle;
            int id = TextUtils.ToInt(grvMaster.GetFocusedRowCellValue("ID"));
            if (id <= 0) return;
        ...
        private void btnSyntheticWorkerPartlist_Click(object sender, EventArgs e)
        {
            int projectID = TextUtils.ToInt(grvMaster.GetFocusedRowCellValue(colIDMaster));
            setJoinerIDs(projectID);
            if (projectID <= 0)
            ...
            ProjectModel project = SQLHelper<ProjectModel>.FindByID(projectID);
            var frm = new frmProjectPartlistAndWorker();
            frm.project = project;
            frm.Show();
            ...
            //}
            //ProjectModel project = SQLHelper<ProjectModel>.FindByID(projectID);
            //frmSyntheticWorkerPartlist frm = new frmSyntheticWorkerPartlist();
            //frm.project = project;
            //frm.isTBP = false;
            //frm.Show();
        }
        ...
        {
            //flyoutPanel1.ShowPopup();
            //var control = flyoutPanelControl1.Controls;
        }
        ...
        private void flyoutPanel1_ButtonClick(object sender, FlyoutPanelButtonClickEventArgs e)
        {
            ...
            {
                case "btnOK":
                    DateTime? dateStatusLog = TextUtils.ToDate4(dtpDateStatusLog.EditValue);
                    if (!dateStatusLog.HasValue)
                    {
                        MessageBox.Show($"Vui lòng chọn {label17.Text}!", "Thông báo");
                        break;
                    }
                    var rowHandle = grvMaster.FocusedRowHandle;
                    int[] selectedRows = grvMaster.GetSelectedRows();
                    foreach (int row in selectedRows)
                    {
                        ...
                        if (projectID <= 0) continue;
                        ProjectModel project = SQLHelper<ProjectModel>.FindByID(projectID);
                        project.ProjectStatus = TextUtils.ToInt(flyoutPanel1.Tag);

                        SQLHelper<ProjectModel>.Update(project);

                        TextUtils.ExcuteProcedure("spUpdateProjectStatus",
                                                    new string[] { "@ProjectID", "@ProjectStatusID" },
                                                    new object[] { project.ID, TextUtils.ToInt(flyoutPanel1.Tag) });
        ...
        frmProjectItemNew frm;
        private void btnProjectItemNewContent_Click(object sender, EventArgs e)
        {
            try
            {
                int ID = TextUtils.ToInt(grvMaster.GetFocusedRowCellValue(colIDMaster));
                ...
                {
                    frm = new frmProjectItemNew();
                    frm.project = model;
                    //frm.Tag = model.ProjectCode;
                    frm.FormClosed += (s, args) => frm = null;
                    frm.SaveEvent += loadProjectItem;
                    frm.Show();
                }
                else
                {
        ...
        private void flyoutPanel2_ButtonClick(object sender, FlyoutPanelButtonClickEventArgs e)
        {
            string tag = e.Button.Tag.ToString();
            switch (tag)
            {
                case "btnOK":
                    cboPM.EditValue = 0;
                    cboProjectType.SetEditValue("");
                    cboProjectStatusNew.SetEditValue("");
        ...
        private void btnMeetingMinutes_Click(object sender, EventArgs e)
        {
            int projectID = TextUtils.ToInt(grvMaster.GetFocusedRowCellValue(colIDMaster));
            ...
            ProjectModel project = SQLHelper<ProjectModel>.FindByID(projectID);
            frmMeetingMinutes frm = new frmMeetingMinutes();
            //frm.ProjectID = projectID;
            //frm.projectCode = projectCode;
            //frm.projectName = projectName;
            frm.project = project;
