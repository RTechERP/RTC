...
namespace BMS
{
    public partial class frmLostReportAsset : _Forms
    {
        public TSLostReportAssetModel lost = new TSLostReportAssetModel();
        ...
        public frmLostReportAsset()
        {
            InitializeComponent();
        }

        private void frmLostReportAsset_Load(object sender, EventArgs e)
        {
            try
            {
        ...
        void LoadLostReport()
        {
            dtpMat.Value = lost.DateLostReport.HasValue == true ? lost.DateLostReport.Value : DateTime.Now;
            txtLyDo.Text = lost.Reason;
        }
        public bool ValidateForm()
        {
            if (txtLyDo.Text == "")
            {
                MessageBox.Show(string.Format("Vui lòng nhập vào lý do bị mất tài sản !"), TextUtils.Caption, MessageBoxButtons.OK, MessageBoxIcon.Error);
                return false;
            }
            return true;
        }
        bool saveData()
        {
            ...
            lost.AssetManagementID = asset.ID;
            lost.DateLostReport = new DateTime(dtpMat.Value.Year, dtpMat.Value.Month, dtpMat.Value.Day);
            lost.Reason = txtLyDo.Text.Trim();
            if (lost.ID > 0)
            {
                //TSLostReportAssetBO.Instance.Update(lost);
                SQLHelper<TSLostReportAssetModel>. Update(lost);
            }
            else
            {
                //lost.ID = (int)TSLostReportAssetBO.Instance.Insert(lost);
                lost.ID = SQLHelper<TSLostReportAssetModel>.Insert(lost).ID;
            }
            ...
            ast.DateAllocation = new DateTime(dtpMat.Value.Year, dtpMat.Value.Month, dtpMat.Value.Day);
