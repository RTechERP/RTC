...
namespace BMS
{
    public partial class frmBonusKPI : _Forms
    {
        public frmBonusKPI()
        {
            InitializeComponent();
        ...
        void loadgrvData()
        {
            ...
            DataTable pe = TextUtils.Select($"Select * From BonusRuleIndex where GroupSalesID = {cbGroup.EditValue} and SaleUserTypeID={cbPosition.EditValue}");
                if (pe.Rows.Count == 0) return;
            txtBase.EditValue = pe.Rows[0]["PercentBase"];
            txtVision.EditValue = pe.Rows[0]["PercentVision"];
            txtBQMS.EditValue = pe.Rows[0]["PercentBQMS"];
            txtMachine.EditValue = pe.Rows[0]["PercentMachine"];
        }
        ...
        private void btnSave_Click(object sender, EventArgs e)
        {
            ...
            bonusIndex.PercentMachine = TextUtils.ToDecimal(txtMachine.EditValue);
            bonusIndex.PercentBQMS = TextUtils.ToDecimal(txtBQMS.EditValue);
            bonusIndex.PercentBase = TextUtils.ToDecimal(txtBase.EditValue);
            bonusIndex.PercentVision = TextUtils.ToDecimal(txtVision.EditValue);
