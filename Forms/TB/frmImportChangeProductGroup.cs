...
namespace BMS
{
    public partial class frmImportChangeProductGroup : _Forms
    {
        public frmImportChangeProductGroup()
        {
            InitializeComponent();
        }
        DateTime start;
        ...
        private void backgroundWorker1_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            for (int i = 0; i < rowCount; i++)
            {
                try
                {
                    ...
                    string MaGroup = Lib.ToString(grvData.GetRowCellValue(i, "F1"));
                    if (string.IsNullOrEmpty(MaGroup.Trim()))
                    {
                        continue;
                    }
                    string MaNB = Lib.ToString(grvData.GetRowCellValue(i, "F3"));
                    TextUtils.ExcuteProcedure("spChangeProductGroup", new string[] { "@MaGroup", "@MaNB" }, new object[] { MaGroup.Trim(), MaNB.Trim() });
                }
                catch (Exception ex)
                {
