...
namespace BMS
{
    public partial class frmTradePrice : _Forms
    {
        ...
        private void frmTradePrice_Load(object sender, EventArgs e)
        {
            ...
            //dtpDateEnd.Value = new DateTime(now.Year, now.Month, DateTime.DaysInMonth(now.Year, now.Month), 23, 59, 59, 999);
        ...
        void loadData()
        {
            ...
            DataTable dt = TextUtils.LoadDataFromSP("spGetTradePrice", "A",
        ...
        void loadDetail()
        {
            ...
            DataTable dt = TextUtils.LoadDataFromSP("spGetTradePriceDetail", "A", new string[] { "@TradePriceID" }, new object[] { id });
        ...
        private void grvData_FocusedRowChanged(object sender, DevExpress.XtraGrid.Views.Base.FocusedRowChangedEventArgs e)
        {
            ...
            bandEXW.Caption = $"EXW = " + TextUtils.ToDecimal(grvData.GetRowCellValue(grvData.FocusedRowHandle, colEXW)).ToString("n2");
            bandMargin.Caption = $"Margin = " + TextUtils.ToDecimal(grvData.GetRowCellValue(grvData.FocusedRowHandle, colMargin)).ToString("n2");
            bandTotalCMPerSet.Caption = $"Tổng CM/Set = " + TextUtils.ToDecimal(grvData.GetRowCellValue(grvData.FocusedRowHandle, colTotalCMPerSET)).ToString("n2");
            bandTotalProfit.Caption = $"Lợi nhuận = " + TextUtils.ToDecimal(grvData.GetRowCellValue(grvData.FocusedRowHandle, colTotalProfit)).ToString("n2");
            bandTotalProfitRate.Caption = $"Tỷ lệ lợi nhuận = " + TextUtils.ToDecimal(grvData.GetRowCellValue(grvData.FocusedRowHandle, colTotalProfitPercentFooter)).ToString("n2");
        }
        ...
        private void btnDelete_Click(object sender, EventArgs e)
        {
            if (grvData.RowCount > 0)
            {
                ...
                DialogResult dialog = MessageBox.Show("Bạn có muốn xóa bảng tính giá thương mại đã chọn không?", TextUtils.Caption, MessageBoxButtons.YesNo, MessageBoxIcon.Question);
                if (dialog == DialogResult.Yes)
                {
                    ...
                    SQLHelper<TradePriceModel>.UpdateFieldsByID(myDict, id);
        ...
        private void btnExportExcel_Click(object sender, EventArgs e)
        {
            ...
            using (WaitDialogForm fWait = new WaitDialogForm("Vui lòng chờ trong giây lát...", "Đang tạo phiếu..."))
            {
                ...
                try
                {
                    ...
                    DataTable dt = (DataTable)tlData.DataSource;
        ...
        private void btnExportRequest_Click(object sender, EventArgs e)
        {
            ...
            foreach (frmBillExportDetailNew frm in formList)
            {
                if (frm.ShowDialog() == DialogResult.OK)
                {
                    loadData();
        ...
        private void AddFormNew(DataTable dtDetailForForm, int customerID, int productGroupID, int saleAdminID, List<frmBillExportDetailNew> formList)
        {
        ...
        private void grvData_RowStyle(object sender, DevExpress.XtraGrid.Views.Grid.RowStyleEventArgs e)
        {
            ...
            int isQuotation = TextUtils.ToInt(grvData.GetRowCellValue(e.RowHandle, colIsQuotation));
            if (isQuotation == 1)
            {
        ...
        private void btnQuotation_Click(object sender, EventArgs e)
        {
            ...
            int id = TextUtils.ToInt(grvData.GetFocusedRowCellValue(colID));
            TradePriceModel tradePrice = SQLHelper<TradePriceModel>.FindByID(id);
            if (tradePrice.IsApprovedBGD == 0)
            {
                MessageBox.Show("Bảng tính giá chưa được BGĐ duyệt!", "Thông báo");
                return;
            }
            if (tradePrice.IsQuotation == 1)
            {
                MessageBox.Show("Bảng tính giá đã được báo giá!", "Thông báo");
                return;
            }
            frmQuotationSaleDetail frm = new frmQuotationSaleDetail();
            frm.tradePrice = tradePrice;
            if (frm.ShowDialog() == DialogResult.OK)
            {
                TradePriceModel trade = SQLHelper<TradePriceModel>.FindByID(id);
                trade.IsQuotation = 1;
                SQLHelper<TradePriceModel>.Update(trade);

                //TextUtils.ExcuteSQL($"UPDATE TradePrice SET IsQuotation = 1 WHERE ID = {id}");
                loadData();
